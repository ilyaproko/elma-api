...
class UploadExcel
{
    ...
    public ElmaApi elmaApi;
    public UploadExcel(BaseHttp baseHttp, ElmaApi elmaApi)
    {
        this.baseHttp = baseHttp; // нужен для простых запросов-http к серверу Elma
        this.elmaApi = elmaApi; // нужен для уже подготовленных запросов к Elma
    }
    ...
    public void academicTitle(string pathExcel)
    {
        ...
        if (!ElmaGuard.existRecord(TypesUidElma.users, "UchyonoeZvanie", "перечисление"))
            throw new Exception("Error existing record object ELMA");

        // получение всех пользоваетелей user из сервера elma
        var getUsersElma = elmaApi.queryEntityList(type: TypesUidElma.users);
        ...
        foreach (Root user in getUsersElma)
        {
            ...
            var id = ElmaApi.getValueItem(user.Items, "Id");
            var fullName = ElmaApi.getValueItem(user.Items, "FullName");
            var userName = ElmaApi.getValueItem(user.Items, "UserName"); // учетная запись
            var academicTitle = ElmaApi.getValueItem(user.Items, "UchyonoeZvanie");
        ...
        while (currentSheet.Cells[currRow, currCell].StringValue != "")
        {
            ...
            if (tryFindUser != null && academicTitleExcel != "")
            {
                ...
                if (tryFindUser.academicTitle != prepareAcadTitle)
                {
                    ...
                    var resp = this.baseHttp.requestElma(
        ...
        var againReqUserElma = elmaApi.queryEntityList(type: TypesUidElma.users);
        againReqUserElma.ForEach(user =>
        {
            var acadTitle = ElmaApi.getValueItem(user.Items, "UchyonoeZvanie");
    ...
    public void addUsers(string pathExcel, BaseHttp baseHttpElma)
    {
        ...
        foreach (Root user in getUsersElma)
        {
            ...
            var id = ElmaApi.getValueItem(user.Items, "Id");
            var fullName = ElmaApi.getValueItem(user.Items, "FullName");
            var userName = ElmaApi.getValueItem(user.Items, "UserName");
            var academicTitle = ElmaApi.getValueItem(user.Items, "UchyonoeZvanie");
            // добавление пользователя в хранилище
            usersELma.Add(new UserElma(id, fullName, userName, academicTitle));
        }
