...
class Log
{
    ...
    static public void Warn(String title, String message)
    {
        ...
        // -> (...) <WARN>
        resultStr += " [[[mediumvioletred bold]warn[/]]]";

        // * Title
        resultStr += $" [mediumvioletred]{title.ToLower()}[/]";
    ...
    static public void UploadDb(UploadResult uploadResult)
    {
        Log.Info(InfoTitle.uploadElma, uploadResult.NameMethodUpload, colorTitle: "orangered1");

        Log.Info(InfoTitle.dataElma, $"{uploadResult.EntitiesElmaBefore} {uploadResult.NameMethodUpload} in elma");

        Log.Info(InfoTitle.dataDB, $"{uploadResult.EntitiesForInject} {uploadResult.NameMethodUpload} in database");

        if (uploadResult.EntitiesMissed != 0)
            Log.Notice(NoticeTitle.missedData, $"{uploadResult.EntitiesMissed} missed {uploadResult.NameMethodUpload}");

        if (uploadResult.EntitiesIdInjected.Count == uploadResult.EntitiesMissed)
            Log.Success(SuccessTitle.synchronized, $"{uploadResult.NameMethodUpload} synchronized");
        else
            Log.Warn(WarnTitle.unsynchronized,
                $"{uploadResult.NameMethodUpload} missed: {uploadResult.EntitiesMissed}, {uploadResult.NameMethodUpload} injected: {uploadResult.EntitiesIdInjected}");

        if (uploadResult.EntitiesIdInjected.Count != 0 && uploadResult.EntitiesIdInjected.Count == uploadResult.EntitiesMissed)
            Log.Success(SuccessTitle.injectedData, $"{uploadResult.EntitiesIdInjected} injected {uploadResult.NameMethodUpload} to elma-server");
    }
}
...
public static class InfoTitle
{
    ...
    public static readonly string updateEntities = "update-entities";
}
...
public static class WarnTitle
{
    ...
    public static readonly string missed = "missed";
    public static readonly string unsynchronized = "unsynchronized";
    public static readonly string faildInserted = "faild-inserted";
    public static readonly string methodProcess = "method-process";
}
